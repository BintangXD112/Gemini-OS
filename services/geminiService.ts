...
export async function* streamAppContent(
  ...
  const model = 'gemini-2.5-flash-preview-04-17'; // Updated model
  ...
  if (
    currentInteraction.type === 'system_event' &&
    currentInteraction.id === 'system_image_generated'
  ) {
    currentInteractionSummary = `Current Event: A new image was generated for prompt: '${currentElementName}'. You MUST display it. Use this as the src: ${currentInteraction.value}`;
  } else {
    currentInteractionSummary = `Current User Interaction: Clicked on '${currentElementName}' (Type: ${
      currentInteraction.type || 'N/A'
    }, ID: ${currentInteraction.id || 'N/A'}).`;
    if (currentInteraction.value) {
      currentInteractionSummary += ` Associated value: '${currentInteraction.value.substring(
        0,
        100,
      )}'.`;
    }
  ...
  const currentAppContext = currentInteraction.appContext
    ? `Current App Context: '${
        currentAppDef?.name || currentInteraction.appContext
      }'.`
  ...
  if (pastInteractions.length > 0) {
    ...
    pastInteractions.forEach((interaction, index) => {
      ...
      if (
        ...
        interaction.id === 'system_image_generated'
      ) {
        historyPromptSegment += `\n${
          index + 1
        }. (App: ${appName}) A new image was generated for prompt '${pastElementName}'. You MUST display it. Use this as the src: ${
          interaction.value
        }`;
      } else {
        historyPromptSegment += `\n${
          index + 1
        }. (App: ${appName}) Clicked '${pastElementName}' (Type: ${
          interaction.type || 'N/A'
        }, ID: ${interaction.id || 'N/A'})`;
        if (interaction.value) {
          historyPromptSegment += ` with value '${interaction.value.substring(
            0,
            50,
          )}'`;
        }
...
export async function generateImage(prompt: string): Promise<string | null> {
  ...
  if (!prompt) {
    console.error('Prompt is required to generate an image.');
    return null;
  }
  ...
  try {
    ...
    if (
      ...
      console.error('Image generation response was empty or malformed.');
      return null;
    }
  } catch (error) {
