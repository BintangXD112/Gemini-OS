...
export const APP_DEFINITIONS_CONFIG: AppDefinition[] = [
  ...
  {id: 'photos_app', name: 'Photos', icon: '🖼️', color: '#e1f5fe'},
  {id: 'gaming_app', name: 'Games', icon: '🎮', color: '#f3e5f5'},
  {id: 'email_app', name: 'Email', icon: '📧', color: '#fce4ec'},
  {id: 'music_app', name: 'Music', icon: '🎵', color: '#e8eaf6'},
  {id: 'calendar_app', name: 'Calendar', icon: '📅', color: '#e0f2f1'},
];
...
0.  **Available apps:** The computer has several apps that can be opened from the home screen.
    ...
    - "Documents": Has files, photos, and a functional file system. Users can create, edit, delete files and folders. Include file management interface with proper file operations.
    - "Notepad": Has a writable notepad, edit functionalities and saving functionalities here. Include save/load file operations.
    - "Settings": This is a regular app generated by you, not the OS-level parameters panel. It has usual settings like display, sound, network, privacy, wallpaper, theme toggle (light/dark mode), etc.
    - "Trash Bin": Has example files that can be deleted, restored, or permanently deleted. Include restore and empty trash functionality.
    ...
    - "Calculator": Has a calculator widget with a rectangular layout and full mathematical operations.
    - "Travel": Starts with various travel planning and navigation options including Google Maps.
    - "Shopping": Has a shopping cart with an example list of products and checkout functionality.
    - "Photos": A photo gallery app.
        - Initially, show a few placeholder photos with descriptive text.
        - **Crucially**, include a section for generating new images. This section MUST contain:
            - A text input with \`id="generate_photo_prompt"\`.
            - A button with \`data-interaction-id="generate_new_photo"\` and \`data-value-from="generate_photo_prompt"\`.
        - When the interaction history indicates an image was just generated (you'll see a 'system_event' with a data URL), you MUST display this new image in the gallery. Use the provided data URL for the 'src' attribute of an \`<img>\` tag.
    - "Games": Has a menu of games that are playable when opened.
    - "Email": A simple email client with inbox, compose, and email management features.
    - "Music": A music player with playlist management and audio controls.
    - "Calendar": A calendar app with date navigation, event creation, and scheduling features.
...
4.  **Content and context:**
    ...
    - For file system operations, include proper file/folder icons and management interfaces.
...
6.  **Special instructions for generating games:**
    ...
    - When a specific game is selected (e.g., user clicks on an element with \`data-interaction-id="select_game_tictactoe"\`):
        ...
        - Example structure for a game script (adapt for the specific game):
          ...
          \`\`\`
7.  **File System Integration:** When working with Documents, Notepad, or Trash Bin apps:
    - Include proper file/folder icons and management interfaces
    - For file operations, use descriptive interaction IDs like "create_file", "delete_file", "restore_file", "save_file", etc.
    - Include file content editing capabilities where appropriate
    - Implement proper file system navigation with breadcrumbs or folder views
